...
<s:if test="%{newAddress}">
	<title><s:text name="msgkey.title.newAddress" /></title>
</s:if>
<s:else>
	<title><s:text name="msgkey.title.editAddress" /></title>
</s:else>
</head>
<body>
	<s:form namespace="/checkout" action="address">
		...
		<s:select id="address_country" required="true"
			...
			list="countries" listKey="code" listValue="fullName" headerKey=""
			headerValue="- Select country -" />
		<s:textfield key="msgkey.label.address.street" name="address.street"
			required="true" />
		<s:textfield key="msgkey.label.address.street2" name="address.street2" />
		<s:textfield key="msgkey.label.address.city" name="address.city"
			required="true" />
		<s:textfield key="msgkey.label.address.state" name="address.state" />
		<s:textfield key="msgkey.label.address.postalCode"
