...
<head>
<s:if test="%{shop != null}">
	<title>Product list by shop "<s:property value="%{shop.name}" />"
	</title>
</s:if>
<s:else>
...
<body>
	<s:if test="!isEmpty(products)">
		<a href="<%= request.getContextPath() %>/member/product!add">Add
			product</a>
		<p />
		<f:topPager />
		<table class="ListTable">
			...
			<s:iterator value="products" status="num">
				<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
					...
					<td>TODO</td>
					<td><s:url action="product" namespace="/member"
							var="productUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:a href="%{#productUrl}">
					...
					<td class="Actions"><s:url action="product"
							namespace="/member" var="viewProductUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="product!edit" namespace="/member"
							var="editProductUrl">
							<s:param name="id" value="%{id}" />
						...
						<ul>
							<li><s:a href="%{#viewProductUrl}">
									<s:text name="msgkey.action.view" />
								</s:a></li>
							<li><s:a href="%{#editProductUrl}">
									<s:text name="msgkey.action.edit" />
								</s:a></li>
							<li><s:a href="%{#deleteProductUrl}">
