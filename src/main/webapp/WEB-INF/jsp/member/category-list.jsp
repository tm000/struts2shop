...
<body>
	<s:if test="!isEmpty(categories)">
		<a href="<%= request.getContextPath() %>/member/category!add">Add
			category</a>
		<p />
		<f:topPager />
		<table class="ListTable">
			...
			<s:iterator value="categories" status="num">
				<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
					...
					<td><s:url action="category" namespace="/member"
							var="categoryUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:a href="%{#categoryUrl}">
					...
					<td class="Actions"><s:url action="category"
							namespace="/member" var="viewCategoryUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="category!edit" namespace="/member"
							var="editCategoryUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="category!deleteConfirm" namespace="/member"
							var="deleteCategoryUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="product!add" namespace="/member"
							var="addProductUrl">
							<s:param name="categoryName" value="%{name}" />
						</s:url>
						<ul>
							<li><s:a href="%{#viewCategoryUrl}">
									<s:text name="msgkey.action.view" />
								</s:a></li>
							<li><s:a href="%{#editCategoryUrl}">
									<s:text name="msgkey.action.edit" />
								</s:a></li>
							<li><s:a href="%{#deleteCategoryUrl}">
