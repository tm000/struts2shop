...
<body>
	...
	<s:else>
		...
		<s:form action="product" enctype="multipart/form-data" method="POST"
			onsubmit="onFormSubmit();">
			<h2>
				<s:text name="msgkey.title.mainInfo" />
			</h2>
			<%-- TODO: Enhancement: print link "Edit" for selected shop. (Updated with select.onchange) --%>
			<s:select id="shopId" name="shopId" required="true" list="getShops()"
				listKey="id" listValue="name + ' (' + currency + ')'" headerKey=""
				headerValue="- Select shop -" onchange="changeShop();"
			...
			<div class="FieldGroup">
				<div class="Caption">
					<table class="HiddenTable">
						<tr>
							<td class="LeftCell"><span class="Label"><s:text
										name="msgkey.title.images" /></span></td>
							<td class="RightCell">
								<div id="imageForm-controls" class="FormCopierControls">
									<input type="button" onclick="imageFormCopier.add(1);"
										value="<s:text name="msgkey.action.small.add"/>" /> <input
										type="button" onclick="imageFormCopier.add(5);"
										value="<s:text name="msgkey.action.small.add5"/>" /> <input
										type="button" id="imageForm-removeAllButton"
										disabled="disabled" onclick="imageFormCopier.removeAll();"
										value="<s:text name="msgkey.action.small.removeAll"/>" />
								</div>
							</td>
						</tr>
					</table>
			...
			<div class="FieldGroup">
				<div class="Caption">
					<table class="HiddenTable">
						<tr>
							<td class="LeftCell"><span class="Label"><s:text
										name="msgkey.title.productVariants" /></span></td>
							<td class="RightCell"><s:set var="hasVariants"
									value="!isEmpty(variants)" />
								<div id="variantForm-controls" class="FormCopierControls">
									<input type="button" onclick="variantFormCopier.add(1);"
										value="<s:text name="msgkey.action.small.add"/>" /> <input
										type="button" onclick="variantFormCopier.add(5);"
										value="<s:text name="msgkey.action.small.add5"/>" /> <input
										type="button" id="variantForm-removeAllButton"
										<s:property value="#hasVariants ? '' : 'disabled=\"disabled\"'"/>
										onclick="variantFormCopier.removeAll();"
										value="<s:text name="msgkey.action.small.removeAll"/>" /> <input
										type="button" onclick="variantFormCopier.toggleAll();"
				...
				<ul id="variantForm-block" class="DndList">
					<s:if test="#hasVariants">
						<s:iterator value="variants" status="num">
							<jsp:include
								page="/WEB-INF/jsp/includes/productVariantPattern.jsp" />
						</s:iterator>
					</s:if>
				</ul>
			</div>
			...
			<div class="FieldGroup">
				<div class="Caption">
					<table class="HiddenTable">
						<tr>
							<td class="LeftCell"><span class="Label"><s:text
										name="msgkey.title.categories" /></span></td>
							<td class="RightCell"><s:set var="hasCategories"
									value="product.productCategories != null && product.productCategories.size > 0" />
								<div id="categoryForm-controls" class="FormCopierControls">
									<input type="button" onclick="categoryFormCopier.add(1);"
										value="<s:text name="msgkey.action.small.add"/>" /> <input
										type="button" onclick="categoryFormCopier.add(5);"
										value="<s:text name="msgkey.action.small.add5"/>" /> <input
										type="button" id="categoryForm-removeAllButton"
										<s:property value="#hasCategories ? '' : 'disabled=\"disabled\"'"/>
										onclick="categoryFormCopier.removeAll();"
										value="<s:text name="msgkey.action.small.removeAll"/>" />
								</div></td>
						</tr>
					</table>
							...
							<div id="categoryForm-<s:property value="#i"/>">
								<s:push value="category">
									...
									<h:formField required="true" labelKey="msgkey.label.category">
										<jsp:attribute name="fieldName">
											<s:property value="%{#categoryName}" />
										</jsp:attribute>
										<jsp:attribute name="inputId">
			...
			<h:formControlPanel>
				<s:if test="newEntity">
					<s:submit cssClass="MainButton" name="method:save"
						key="msgkey.action.save" />
					<s:submit name="method:saveAdd" key="msgkey.action.saveAndAddMore" />
				</s:if>
				<s:else>
					<s:hidden name="id" value="%{id}" />
					<s:submit cssClass="MainButton" name="method:update"
						key="msgkey.action.update" />
					<s:url action="product!deleteConfirm" namespace="/member"
						var="deleteUrl">
						<s:param name="id" value="%{id}" />
					</s:url>
					<s:a href="%{#deleteUrl}">
						<s:text name="msgkey.action.delete" />
					</s:a>
				</s:else>
			</h:formControlPanel>
		</s:form>
	...
	<div id="formPatterns" style="display: none;">
		...
		<div id="variantForm-pattern" class="FormPattern"
			style="display: none;">
			<jsp:include page="/WEB-INF/jsp/includes/productVariantPattern.jsp" />
		</div>
		<div id="categoryForm-pattern" class="FormPattern"
			style="display: none;">
			<div id="categoryForm-%id%" style="display: none;">
				<h:formField fieldName="categories[%id%].name"
					inputId="categories_%id%_name" labelKey="msgkey.label.category">
					<table class="InlineHiddenTable">
						<tr>
							<td><s:textfield id="categories_%id%_name"
									name="categories[%id%].name" theme="simple" /></td>
	...
	<content tag="lastJavaScript"> <script type="text/javascript"
		src="<%= request.getContextPath() %>/lib/fckeditor/2.6.4.1/fckeditor.js">
			</script> <%-- TODO: PROD: Include only required jQuery UI libs? (y) --%> <script
		...
		src="<%= request.getContextPath() %>/lib/jquery-ui/1.7.2/js/jquery-ui-1.7.2.custom.min.js">
				...
				// key - shop ID;
				// value - currency code.
				var shopIdToCurrencyJsonMap = <s:property value="shopIdToCurrencyJsonMap" escapeHtml="false"/>;
