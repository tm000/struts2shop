...
<body>
	...
	<s:else>
		...
		<s:form action="shop" enctype="multipart/form-data" method="POST">
			...
			<h:formField inputId="shop_urlName" fieldName="shop.urlName"
				labelKey="msgkey.label.urlName">
				<div class="UrlNameBlock">
					<s:property value="%{urlBuilder.viewShopFullBase}" />
					/
					<s:textfield id="shop_urlName" name="shop.urlName" theme="simple" />
				</div>
				<div class="FormFieldHint">
					<s:text name="msgkey.text.formFieldHint.validUrlName" />
				</div>
			</h:formField>

			<%-- TODO: Low: Country label must be more descriptive. Current label is "Country". --%>
			<s:select name="shop.countryCode" required="true" list="countries"
				listKey="code" listValue="fullName" headerKey=""
				headerValue="- %{getText('msgkey.option.defailt.choose.country')} -"
				key="msgkey.label.country" />

			<s:select name="shop.currency" required="true" list="currencies"
				listKey="code" listValue="fullName" headerKey=""
				headerValue="- %{getText('msgkey.option.defailt.choose.currency')} -"
				key="msgkey.label.currency" />

			<%-- TODO: Low: Make logo editing more beautiful. --%>
			<h:formField inputId="shop_logo" fieldName="logo"
				labelKey="msgkey.label.logo">
				<s:hidden id="removeLogo" name="deleteLogo" value="false" />
				<s:set var="shopHasLogo" value="%{shop.logo != null}" />
				<div id="uploadLogoBox"
					<s:property escapeHtml="false" value="%{#shopHasLogo ? 'style=\"display: none;\"' : ''}"/>>
				...
				<s:if test="#shopHasLogo">
					<div id="logoBox">
						<div>
							<a href="#" onclick="changeLogo();"><s:text
									name="msgkey.action.change" /></a>, <a href="#"
								onclick="removeLogo();"><s:text name="msgkey.action.delete" /></a>
						</div>
						<div style="margin-top: 6px;">
							<%-- TODO: Duplication: Extract to include file? E.g. "print-shop-logo-s200.jsp". --%>
							<s:url var="shopLogoS200Url"
								value="%{'/uploads/images/shop-logos/' + shop.logo.id + '/s200/' + shop.logo.fileName + '.jpg'}"></s:url>
			...
			<h:formControlPanel>
				...
				<s:else>
					...
					<s:url action="shop!deleteConfirm" namespace="/member"
						var="deleteUrl">
