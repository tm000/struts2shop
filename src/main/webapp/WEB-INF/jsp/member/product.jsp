		...
		<s:push value="product">
			<table class="ViewTable">
				...
				<tr>
					...
					<td><s:if test="!isEmpty(productVariants)">
							<ul>
								<s:iterator value="productVariants">
									<%-- TODO: Link to product variant. --%>
									<%-- TODO: UI: Make more beauty. --%>
									<li><s:property value="name" />&nbsp;<s:property
											value="price" />&nbsp;<s:property value="currency" /></li>
								</s:iterator>
							</ul>
						</s:if></td>
				</tr>
				<tr>
					...
					<td><s:if test="!isEmpty(productCategories)">
							<ul>
				...
				<tr>
					...
					<td><s:url action="product!edit" namespace="/member"
							var="editProductUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="product!deleteConfirm" namespace="/member"
							var="deleteProductUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="product!view" namespace="/" var="viewProductUrl">
							<s:param name="productId" value="%{id}" />
						</s:url> <s:a href="%{#editProductUrl}">
							<s:text name="msgkey.action.edit" />
						</s:a>, <s:a href="%{#deleteProductUrl}">
							<s:text name="msgkey.action.delete" />
						</s:a>, <s:a href="%{#viewProductUrl}">
