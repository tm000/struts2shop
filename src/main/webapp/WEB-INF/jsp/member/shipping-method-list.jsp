		...
		<table class="ListTable">
			...
			<s:iterator value="shippingMethods" status="num">
				<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
					...
					<td><s:url action="shipping-method" namespace="/member"
							var="shippingMethodUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:a href="%{#shippingMethodUrl}">
							<s:property value="name" />
						</s:a></td>
					<td class="Price"><s:property value="price" />&nbsp;<s:property
							value="currency" /></td>
					<td><s:url action="shop" namespace="/member" var="shopUrl">
							<s:param name="id" value="%{shop.id}" />
						</s:url> <s:a href="%{#shopUrl}">
							<s:property value="shop.name" />
						</s:a></td>
					<td class="Date"><s:date name="created" /></td>
					<td class="Date"><s:date name="updated" /></td>
					<td class="Actions"><s:url action="shipping-method"
							namespace="/member" var="viewShippingMethodUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="shipping-method!edit" namespace="/member"
							var="editShippingMethodUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="shipping-method!deleteConfirm" namespace="/member"
							var="deleteShippingMethodUrl">
							<s:param name="id" value="%{id}" />
						</s:url>
						<ul>
							<li><s:a href="%{#viewShippingMethodUrl}">
									<s:text name="msgkey.action.view" />
								</s:a></li>
							<li><s:a href="%{#editShippingMethodUrl}">
									<s:text name="msgkey.action.edit" />
								</s:a></li>
							<li><s:a href="%{#deleteShippingMethodUrl}">
