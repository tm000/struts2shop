			...
			<table class="ViewTable">
				...
				<tr>
					...
					<td><s:if test="%{logo != null}">
						...
						</s:if> <s:else>TODO: No logo.</s:else></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="UrlNameBlock"><s:if test="%{!isEmpty(urlName)}">
							<s:a href="%{#shopUrl}">
								<s:property value="%{urlBuilder.viewShopFullBase}" />/<strong><s:property
										value="urlName" /></strong>
							</s:a>
						</s:if></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><s:property value="htmlDescription" escapeHtml="false" /></td>
				</tr>
				<tr>
					...
					<td><s:if test="productCount > 0">
							<s:url action="product!list" namespace="/member"
								var="productsByShopIdUrl">
								<s:param name="shopId" value="%{id}" />
							</s:url>
							<s:a href="%{#productsByShopIdUrl}">
				...
				<tr>
					...
					<td><s:url action="shop!edit" namespace="/member"
							var="editShopUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:url action="shop!deleteConfirm" namespace="/member"
							var="deleteShopUrl">
							<s:param name="id" value="%{id}" />
						</s:url> <s:a href="%{#editShopUrl}">
							<s:text name="msgkey.action.edit" />
						</s:a>, <s:a href="%{#deleteShopUrl}">
							<s:text name="msgkey.action.delete" />
						</s:a>, <s:a href="%{#shopUrl}">
							<s:text name="msgkey.link.viewInAction" />
						</s:a></td>
				</tr>
			</table>
		</s:push>
