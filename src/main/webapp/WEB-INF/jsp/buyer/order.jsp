...
<body>
	...
	<a href="<%= request.getContextPath() %>/buyer/order!list">Back to
		orders</a>
	<s:if test="order == null">
		<%-- TODO: Print this text in title instead. --%>
		<%-- TODO: Print the link to Order list. --%>
		<p>
			<s:text name="msgkey.text.notFound.order" />
		</p>
	</s:if>
	<s:else>
		<s:push value="order">
			...
			<table class="HiddenTable OrderInfoTable">
				<tr>
					<td class="OrderInfoCell">
						<h2>
							<s:text name="msgkey.title.orderSummary" />
						</h2>
						<table class="ViewTable">
							...
							<tr>
								...
								<td>
									<%-- TODO: Duplication block: Extract include file. --%> <s:if
										test="shop != null">
								...
								<td>
									<%-- TODO: Duplication block: Extract include file. --%> <s:if
										...
										<s:a href="%{#viewUserUrl}">
											<s:property value="shop.user.fullName" />
										</s:a>
									</s:if> <s:else>[DELETED]</s:else>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><s:property value="totalPrice" />&nbsp;<s:property
										value="currency" /></td>
							</tr>
							<tr>
							...
							<tr>
								<th><s:text
										name="msgkey.label.totalQuantityOfOrderedProducts" /></th>
					...
					<td class="OrderInfoCell">
						<h2>
							<s:text name="msgkey.title.billingAddress" />
						</h2> <s:set var="address" value="%{billingAddress}" /> <jsp:include
							page="/WEB-INF/jsp/includes/address.jsp" />
					</td>
					<td class="OrderInfoCell">
						<h2>
							<s:text name="msgkey.title.shippingAddress" />
						</h2> <s:set var="address" value="%{shippingAddress}" /> <jsp:include
							page="/WEB-INF/jsp/includes/address.jsp" />
					</td>
				</tr>
			</table>
			...
			<s:if test="%{hasShippingMethod()}">
				<p />
				<h2>
					<s:text name="msgkey.title.shippingMethod" />
				</h2>
			...
			<s:if test="%{hasPaymentMethod()}">
				<p />
				<h2>
					<s:text name="msgkey.title.paymentMethod" />
				</h2>
		...
		<table class="ListTable">
			...
			<s:iterator value="orderItems" status="num">
				...
				<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
					...
					<td>
						<%-- TODO: Duplication: See [checkout/review-order.jsp] --%> <a
						class="ThumbnailLink" href="<s:property value="#viewProductUrl"/>">
							<s:if test="image != null">
								...
								<img
									src="<%= request.getContextPath() %>/images/no-product-image-s40.png">
							</s:else>
					</a>
					</td>
					<td><s:if test="product != null">
							<s:a href="%{#viewProductUrl}">
					...
					<td class="Number"><s:property value="price" />&nbsp;<s:property
							value="currency" /></td>
					<td class="Number"><s:property value="quantity" /></td>
					<td class="Number"><s:property value="subtotal" />&nbsp;<s:property
							value="currency" /></td>
					<td class="Date"><s:date name="created" /></td>
					<td class="Actions">TODO: Smth useful.</td>
				</tr>
			</s:iterator>
		</table>
		<f:bottomPager />
	...
	<p />
	<a href="<%= request.getContextPath() %>/buyer/product!list">Back
		to orders</a>
</body>
</html>