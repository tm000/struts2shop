...
<body>
	<s:form action="login">
		...
		<h:formField fieldName="password" required="true"
			labelKey="msgkey.label.password">
			<input type="password" id="password" name="password" />
			<a href="#TODO: reset-password"><s:text
					name="msgkey.link.question.forgotYourPassword" /></a>
		</h:formField>
		...
		<h:formField fieldName="rememberMe">
			...
			<label for="rememberMe"><s:text
					name="msgkey.text.rememberThisLogin" /></label>
		</h:formField>

		<h:formControlPanel>
			<s:submit cssClass="MainButton" name="method:submit"
				key="msgkey.action.login" />
			<s:url var="registerUrl" action="register" namespace="/" />
			<s:a href="%{#registerUrl}">
				<s:text name="msgkey.link.question.notRegistered" />
			</s:a>
		</h:formControlPanel>
	</s:form>
