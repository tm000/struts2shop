...
<html>
<head>
<title><s:text name="msgkey.title.searchResultsFor">
		<s:param value="%{query}" />
	</s:text></title>
</head>
<body>
	<s:url var="shoppingCountriesUrl" namespace="/"
		action="shopping-countries" />
	<%-- TODO: i18n --%>
	<h2>
		<s:text name="msgkey.title.selectShoppingCountries" />
		(
		<s:a href="%{#shoppingCountriesUrl}">
			<s:text name="msgkey.action.change" />
		</s:a>
		)
	</h2>
			...
			<table class="ListTable">
				...
				<s:iterator value="products" status="num">
					<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
						<td>TODO</td>
						<td>TODO</td>
						<td><s:url action="product!view" namespace="/"
								var="viewProductUrl">
								<s:param name="productId" value="%{id}" />
							</s:url> <s:a href="%{#viewProductUrl}">
						...
						<td><s:set var="shopUrl"
								value="%{urlBuilder.getViewShop(shop)}" /> <s:a
								href="%{#shopUrl}">
	...
	<s:if test="isSearchIn('shops')">
		<h2>
			<s:text name="msgkey.title.shops" />
		</h2>
				...
				<s:iterator value="shops" var="shop" status="num">
					<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
						...
						<td><a class="ThumbnailLink"
							href="<s:property value="#shopUrl"/>"> <s:if
									test="%{logo != null}">
									...
									<img
										src="<%= request.getContextPath() %>/images/no-shop-logo-s40.png">
								</s:else>
						</a></td>
						<td><s:a href="%{#shopUrl}">
								<s:property value="name" />
						...
						<td><s:url action="user" namespace="/" var="viewUserUrl">
