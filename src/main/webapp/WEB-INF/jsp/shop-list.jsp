		...
		<table class="ListTable">
			...
			<s:iterator value="shops" var="shop" status="num">
				<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
					...
					<%-- TODO: Usability: Remove actions block? (y) --%>
					<s:if test="loggedIn">
						<td class="Actions"><s:if test="user.equals(currentUser)">
								...
								<s:url action="product!add" namespace="/member"
									var="addProductUrl">
									<s:param name="shopId" value="%{id}" />
								</s:url>
								<ul>
									<li><s:a href="%{#viewShopUrl}">
											<s:text name="msgkey.action.view" />
										</s:a></li>
									<li><s:a href="%{#editShopUrl}">
											<s:text name="msgkey.action.edit" />
										</s:a></li>
									<li><s:a href="%{#deleteShopUrl}">
