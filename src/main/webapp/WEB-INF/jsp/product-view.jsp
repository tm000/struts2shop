...
<body>
	<s:if test="%{product != null}">
		<s:push value="product">
			<table class="HiddenTable">
				<tr class="TopRow">
					<td>
						<h2>
							<s:text name="msgkey.title.images" />
						</h2> <s:if test="titleImage != null">
							<%-- TODO: Low: Integrate images w/ Fancybox jQuery plugin. --%>
							<table>
								<tr>
									<td><s:iterator value="productImages" status="num">
											<a class="ThumbnailLink"
												href="<%= request.getContextPath() %>/uploads/images/products/<s:property value="image.id + '/' + image.fileName"/>"
												...
												<img
												src="<%= request.getContextPath() %>/uploads/images/products/<s:property value="image.id + '/small/' + image.fileName + '.jpg'"/>">
											</a>
								...
								<tr>
									<td>
										...
										<a class="LargeThumbA ThumbnailLink"
										href="<%= request.getContextPath() %>/uploads/images/products/<s:property value="titleImage.id + '/' + titleImage.fileName"/>">
											<table>
												<tr>
													<td class="LargeThumbFrame">
														<%-- TODO: Duplication: Extract to include file? E.g. "print-product-image-s300.jsp". --%>
														<img class="LargeThumbImg"
														src="<%= request.getContextPath() %>/uploads/images/products/<s:property value="titleImage.id + '/large/' + titleImage.fileName + '.jpg'"/>">
													</td>
												</tr>
											</table>
									</a>
							...
							<%-- TODO: Fix: Enhance. --%>
							<img
								src="<%= request.getContextPath() %>/images/no-product-image-s300.png">
						</s:else>
					</td>
					<td style="padding-left: 6px;">
						<h2>
							<s:text name="msgkey.title.summary" />
						</h2> <s:property value="summary" />

						<h2>
							<s:text name="msgkey.title.productVariants" />
						</h2> <s:if test="%{!isEmpty(productVariants)}">
							<table class="ListTable">
								...
								<s:iterator value="productVariants" status="num">
									<s:if test="%{inStock}">
										<tr
											class="<s:property value="%{(#i % 2 != 0) ? 'Odd' : 'Even'}"/>">
											<td><s:property value="name" /></td>
											<td class="OldPrice"><s:if test="%{oldPrice != null}">
													<s:property value="oldPrice" />&nbsp;<s:property
														value="currency" />
												</s:if></td>
											<td class="Price"><s:if test="%{price != null}">
													<s:property value="price" />&nbsp;<s:property
														value="currency" />
												</s:if></td>
											<td class="AddToCartBlock"><s:if test="%{inStock}">
													<s:form namespace="/ajax" action="cart!addProductVariant"
														...
														<s:textfield cssClass="QuantityInput" name="quantity"
															value="1" />
														<s:submit cssClass="CartButton"
															key="msgkey.action.addToCart" />
													</s:form>
												</s:if></td>
										</tr>
							...
							<s:text
								name="msgkey.text.cannotBeAddedToCartBecauseProductHasNoVariants" />
						</s:else>
					</td>
				</tr>
			</table>
			...
			<h2>
				<s:text name="msgkey.title.description" />
			</h2>
			<s:property value="htmlDescription" escapeHtml="false" />

			<h2>
				<s:text name="msgkey.title.shop" />
			</h2>
			...
			<h2>
				<s:text name="msgkey.title.categories" />
			</h2>
		...
		<content tag="lastJavaScript"> <script
