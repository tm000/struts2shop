...
<body>
	...
	<s:else>
			...
			<table class="ViewTable">
				<tr>
					...
					<td><s:if test="%{logo != null}">
							...
							<img
								src="<%= request.getContextPath() %>/uploads/images/shop-logos/<s:property value="shop.logo.id + '/s200/' + shop.logo.fileName + '.jpg'"/>">
						</s:if> <s:else>
							<img
								src="<%= request.getContextPath() %>/images/no-shop-logo-s200.png">
						</s:else></td>
				</tr>
				<tr>
		...
		<p />
		<h2>
			<s:text name="msgkey.label.description" />
		</h2>
		<s:property value="shop.htmlDescription" escapeHtml="false" />

		<p />
		<h2>
			<s:text name="msgkey.title.featuredProducts" />
		</h2>
			...
			<table class="ListTable">
				...
				<s:iterator value="featuredProductVariants" status="num">
					...
					<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
						<td class="Price"><s:property value="price" />&nbsp;<s:property
								value="currency" /> <%-- TODO: Need? (n)
								<br/>
								<s:property value="oldPrice"/>&nbsp;<s:property value="currency"/>
								--%></td>
						<td><a class="ThumbnailLink"
							href="<s:property value="#viewProductUrl"/>"> <s:if
									test="titleImage != null">
									<img
										src="<%= request.getContextPath() %>/uploads/images/products/<s:property value="titleImage.id + '/medium/' + titleImage.fileName + '.jpg'"/>">
								</s:if> <s:else>
									<img
										src="<%= request.getContextPath() %>/images/no-product-image-s100.png">
								</s:else>
						</a></td>
						<td><s:a cssClass="ProductNameLink" href="%{#viewProductUrl}">
								<s:property value="fullName" />
							</s:a> <jsp:include page="/WEB-INF/jsp/includes/summaryInListTable.jsp" />
							<%-- TODO: Impl: Button "Add to Cart". --%></td>
					</tr>
				</s:iterator>
			</table>
		</s:else>
		...
		<p />
		<h2>
			<s:text name="msgkey.title.lastAddedProducts" />
		</h2>
		...
		<h2>
			<s:text name="msgkey.title.allShopProducts" />
		</h2>
		...
		<s:a href="%{#shopProductsUrl}">
			<s:text name="msgkey.link.viewAllShopProducts" />
		</s:a>
		...
		<s:else>
			<table class="ListTable">
				...
				<s:iterator value="categories" status="num">
					<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
						<td><s:url namespace="/" action="shop!products"
								var="shopCategoryUrl">
								<s:param name="shopId" value="%{shop.id}" />
								<s:param name="categoryName" value="%{name}" />
							</s:url> <s:a href="%{#shopCategoryUrl}">
		...
		<s:push value="shop">
			...
			<%-- TOOD: Low: UI: Add "add" action link if logged in user is shop owner --%>
			<h2>
				<s:text name="msgkey.title.shippingMethods" />
			</h2>
			...
			<s:else>
				<table class="ListTable">
					...
					<s:iterator value="shippingMethods" status="num">
						<tr
							class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
							<td>
								<div class="TitleInCell">
									<s:property value="name" />
								</div> <s:property value="htmlDesc" escapeHtml="false" />
							</td>
							<td class="Price"><s:property value="price" />&nbsp;<s:property
									value="currency" /></td>
						</tr>
			...
			<%-- TOOD: Low: UI: Add "add" action link if logged in user is shop owner --%>
			<h2>
				<s:text name="msgkey.title.paymentMethods" />
			</h2>
