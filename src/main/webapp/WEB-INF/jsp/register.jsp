...
<body>
	<s:form action="register">
		...
		<s:textfield id="firstName" name="firstName" required="true"
			key="msgkey.label.firstName" />
		<s:textfield name="lastName" required="true"
			key="msgkey.label.lastName" />
		<s:textfield id="email" name="email" required="true"
			key="msgkey.label.email" />
		...
		<s:password name="password" required="true"
			key="msgkey.label.password" />
		<s:password name="passwordControl" required="true"
			key="msgkey.label.retypePassword" />

		<h:formControlPanel>
			<s:submit cssClass="MainButton" name="method:submit"
				key="msgkey.action.register" />
			<s:url var="loginUrl" action="login" namespace="/" />
			<s:a href="%{#loginUrl}">
				<s:text name="msgkey.link.question.alreadyRegistered" />
			</s:a>
			<p />
			...
			<%-- TODO: i18n: Externalize this big message. Include links (<a>) into message. --%>
				By clicking button &quot;<strong><s:text
					name="msgkey.action.register" /></strong>&quot;, 
				you are agreeing to <s:a href="%{#termsOfServiceUrl}">
				<s:text name="msgkey.link.termsOfService" />
			</s:a>  
				and <s:a href="%{#privacyPolicyUrl}">
				<s:text name="msgkey.link.privacyPolicy" />
			</s:a>.
			</h:formControlPanel>
	</s:form>
