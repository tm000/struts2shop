...
<html>
<head>
<title><s:text name="msgkey.title.cartItemsByShop">
...
<body>
	...
	<s:else>
		<s:form action="cart!update" namespace="/" theme="simple">
				...
				<s:iterator value="cartItems" status="num">
					...
					<tr class="<s:property value="#num.odd == true ? 'Odd' : 'Even'"/>">
						...
						<td class="Number"><s:property value="price" />&nbsp;<s:property
								value="currency" /></td>
						<td class="Number"><s:hidden name="cartItemIds" value="%{id}" />
							<s:textfield cssClass="QuantityInput" name="quantities"
								value="%{quantity}" /></td>
						...
						<td class="Actions">
							<ul>
								<s:url action="cart!remove" namespace="/"
									var="removeFromCartUrl">
									<s:param name="shopId" value="%{shopId}" />
									<s:param name="cartItemId" value="%{id}" />
								</s:url>
								<li><s:a href="%{#removeFromCartUrl}">
										<s:text name="msgkey.action.remove" />
									</s:a></li>
							</ul>
						</td>
					</tr>
			...
			<s:a href="%{#beginCheckoutUrl}">
				<s:text name="msgkey.action.beginCheckout" />
			</s:a>
			<br />
		</s:form>
	</s:else>
