	...
	<s:else>
		<table class="ContentTable">
			<tr>
						...
						<s:push value="shop">
							...
							<div class="LeftPanelBody">
								<div class="ShopLogoBox">
									<a href="<s:property value="#shopUrl"/>"> <s:if
											test="%{logo != null}">
											...
											<img
												src="<%= request.getContextPath() %>/uploads/images/shop-logos/<s:property value="logo.id + '/s200/' + logo.fileName + '.jpg'"/>">
										</s:if> <s:else>
											<img
								...
								<table class="ViewTable LeftPanelTable" style="width: 100%;">
									...
									<s:if test="%{loggedIn && user.equals(currentUser)}">
										<s:url action="shop!edit" namespace="/member"
											var="editShopUrl">
											<s:param name="id" value="%{id}" />
										</s:url>
										<tr>
											<th><s:text name="msgkey.label.actions" /></th>
											<td><s:a href="%{#editShopUrl}">
													<s:text name="msgkey.action.edit" />
												</s:a></td>
										</tr>
									</s:if>
								</table>
						...
						<div class="LeftPanelBody">
							<table class="CategoryTable">
								<s:if test="isEmpty(categories)">
									...
									<tr>
										<td class="Odd"><s:text
												name="msgkey.text.notExist.category" /></td>
									</tr>
								</s:if>
								<s:else>
									<s:iterator value="categories" status="row">
										<tr class="<s:property value="#row.odd ? 'Odd' : 'Even'"/>">
											<td>
												...
												<s:url action="shop!products" namespace="/"
													var="shopProductsUrl">
													...
													test="categoryName != null && name.equalsIgnoreCase(categoryName)">
											...
											<td class="ProductCount"><s:property
				...
				<td class="CenterContentColumn"><f:topPager /> <s:set
						var="first" value="%{pager.firstResultNumber}" /> <s:set
						var="last" value="%{pager.lastResultNumber}" />
						...
						<table class="ProductTable">
							<s:iterator value="%{products}" status="row">
								...
								<tr class="<s:property value="#rowClass"/>">
									<td class="LeftProductCell"><span class="RowNumber"><s:property
												value="#first + #row.index" />.</span></td>
									<td class="ThumbnailCell"><a class="ThumbnailLink"
									...
									<td>
										<div class="ProductNameBlock">
											<s:a cssClass="ProductNameLink" href="%{#viewProductUrl}">
												<s:property value="name" />
											</s:a>
										</div> <jsp:include
											page="/WEB-INF/jsp/includes/summaryInListTable.jsp" /> <%-- TODO: Fix & Update to Seam. --%>
										<%--
									...
									<td class="RightProductCell">
										...
										<div class="Price">
											<s:if test="%{(minPrice != null) and (minPrice == maxPrice)}">
												<s:property value="minPrice" />&nbsp;<s:property
													value="currency" />
											</s:if>
											<s:elseif test="%{(minPrice != null) and (maxPrice != null)}">
												...
												<s:property value="minPrice" />&nbsp;<s:property
													value="currency" /> &#150; 
													<s:property value="maxPrice" />&nbsp;<s:property
													value="currency" />
											</s:elseif>
											<s:else>
